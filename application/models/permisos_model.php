...
class Permisos_model extends CI_Model {
    ...
    function paginado() {
        ...
        if ($rst->num_rows() > 0) {
            foreach ($rst->result() as $row) {
                ...
                $gear.= '<li>' . anchor('permisos/edit/' . $row->id, 'Editar') . '</li>';
                $gear.= '</ul>';
                $gear.= '</div>';
                $f[] = array(
                    $row->id,
    ...
    public function lista_permisos_por_modulo($m) {
        $qry = "select * from permisos where status='activo' and modulo_fkey=$m order by orden";
        $rst = $this->db->query($qry);
        if ($rst->num_rows() > 0) {
            return $rst->result();
        }
